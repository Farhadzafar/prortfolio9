export const translations = {
  en: {
    welcome: "Welcome",
    about: "About",
    services: "Services",
    contact: "Contact",
    portfolio: "Portfolio",
    description: "A modern web application with language and theme support",
    toggleTheme: "Toggle theme",
    changeLanguage: "Change language",
    heroSubtitle: "Dynamic Web Magic with Next.js",
    heroTitle: "Crafting Concepts into Flawless User Experiences",
    heroDescription:
      "Hi! We are a team of full-stack developers, designers, and DevOps experts, dedicated to delivering outstanding digital solutions",
  },
  ar: {
    welcome: "مرحباً",
    about: "نبذة عنا",
    services: "الخدمات",
    contact: "اتصل بنا",
    portfolio: "محفظة الأعمال",
    description: "تطبيق ويب حديث مع دعم اللغة والسمة",
    toggleTheme: "تبديل السمة",
    changeLanguage: "تغيير اللغة",
    heroSubtitle: "سحر الويب الديناميكي مع Next.js",
    heroTitle: "صياغة المفاهيم وتحويلها إلى تجارب مستخدم خالية من العيوب",
    heroDescription:
      "مرحبًا! نحن فريق من المطورين والمصممين وخبراء DevOps، ملتزمون بتقديم حلول رقمية متميزة",
  },
} as const;
